...
public void error(Object e){Console.ForegroundColor=ConsoleColor.Red;Console.WriteLine(e);Console.ResetColor();}
public class BinaryWriter2 extends BinaryWriter {
	public BinaryWriter2(System.IO.Stream stream){super(stream);}
	public void WriteDelta(Object i){if((i%1)!==0){i=Math.round(i);}
	...
	public void WriteDummy(Object i){
		if(i===0){return;}
		WriteDelta(i);
		WriteBytes([0x5D,0]);
	}
}
public class BinaryReader2 extends BinaryReader {
	public BinaryReader2(System.IO.Stream stream){super(stream);}
	public Int32 ReadInt32(){return BitConverter.ToInt32(super.ReadBytes(4).reverse(),0);}
	public UInt32 ReadUInt32(){return BitConverter.ToUInt32(super.ReadBytes(4).reverse(),0);}
	public Int16 ReadInt16(){return BitConverter.ToInt16(super.ReadBytes(2).reverse(),0);}
	public UInt16 ReadUInt16(){return BitConverter.ToUInt16(super.ReadBytes(2).reverse(),0);}
	public Int64 ReadInt64(){return BitConverter.ToInt64(super.ReadBytes(8).reverse(),0);}
	public UInt64 ReadUInt64(){return BitConverter.ToUInt64(super.ReadBytes(8).reverse(),0);}
}
...
public void main(){
	...
	FileStream midS=File.Open(nme,FileMode.Open,FileAccess.Read);
		BinaryReader2 mid=new BinaryReader2(midS);
		FileStream bgmS=File.Open(nme+".bgm",FileMode.Create,FileAccess.Write);
		BinaryWriter2 bgm=new BinaryWriter2(bgmS);
		trackC:byte,ppqn:UInt16;
		byte lKey=255,byte lVelocity=64;
	...
	try{
		...
		if(ppqn!==48){//KH1 forces PPQN=48, even when you save something else
			...
			Console.WriteLine("Modded PPQN: {0} (calc={2})DeltaModDiv: {1}",48,deltaMod,ppqn/deltaMod);
			Console.ResetColor();
			ppqn=48;
		}

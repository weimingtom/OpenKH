...
namespace IDX_Tools
{
    internal class IDXFile : IDisposable, IEnumerable<IDXFile.IDXEntry>
    {
        ...
        private IDXEntry ReadEntry(long index = -1)
        {
            if (index >= 0)
            {
                Position = (uint) index;
    ...
    internal class IMGFile : IDisposable
    {
        ...
        public IMGFile(Stream file, long offset = 0)
        {
            this.file = file;
            this.offset = offset;
        }
    ...
    internal class IDXIMGWriter : IDisposable
    {
        ...
        public IDXIMGWriter(Stream img, long imgoffset = 0)
        {
            this.img = new IMGFile(img, imgoffset);
        }

        private IMGFile img { get; set; }

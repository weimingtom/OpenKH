...
namespace vcBinTex4
{
	public class Reform4
	{
			...
			for (int i = 0; i < 128 * bh; i += 128)
			{
				for (int j = 0; j < 128 * bw; j += 128)
				{
					...
					for (int k = 0; k < 128; k += 16)
					{
						...
						{
							int num2 = 256 * (int)Reform4.tbl4bc[l / 32 + 4 * (k / 16)];
							for (int m = 0; m < 4; m++)
							{
								int num3 = 64 * m;
								int[] array2 = ((m & 1) == 0) ? Reform4.tbl4col0 : Reform4.tbl4col1;
								for (int n = 0; n < 128; n++)
								{
									int num4 = array2[n] / 8;
									int num5 = array2[n] % 8;
									byte b = (byte)(bin[num + num2 + num3 + num4] >> num5 & 15);
									int num6 = j + l + n % 32;
		...
		public static void Encode4b(byte[] src, byte[] gsram, int rrw, int rrh, int ddax, int dday, int baseoff, int bw128)
		{
			for (int i = 0; i < rrh; i++)
			{
				...
				for (int j = 0; j < rrw; j++)
				{
					...
					int num4 = 256 * (int)Reform4.tbl4bc[(num2 & 127) / 32 + 4 * ((num & 127) / 16)];
					int num5 = (num & 15) / 4;
					int num6 = 64 * num5;
					int[] array = ((num5 & 1) == 0) ? Reform4.tbl4col0 : Reform4.tbl4col1;
		...
		{
			byte[] array = new byte[(cx * cy + 1) / 2];
			for (int i = 0; i < cy; i++)
			{
				for (int j = 0; j < cx; j++)
				{
					int num = 8192 * (j / 128 + bw128 * (i / 128));
					int num2 = 256 * (int)Reform4.tbl4bc[(j & 127) / 32 + 4 * ((i & 127) / 16)];
					int num3 = (i & 15) / 4;
					int num4 = 64 * num3;
					int[] array2 = ((num3 & 1) == 0) ? Reform4.tbl4col0 : Reform4.tbl4col1;
					int num5 = (j & 31) + 32 * (i & 3);
					int num6 = array2[num5] / 8;
					int num7 = array2[num5] % 8;
					byte b = (byte)(gsram[readoff + num + num2 + num4 + num6] >> num7 & 15);
					int num8 = j + cx * i;
					byte b2 = array[num8 / 2];
					if ((num8 & 1) == 0)
					{

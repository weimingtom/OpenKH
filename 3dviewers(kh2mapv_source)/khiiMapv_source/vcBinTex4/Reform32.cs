...
namespace vcBinTex4
{
	...
	{
		private static readonly byte[] tbl32pao = new byte[]
		{
			0,
			1,
			4,
			...
			for (int i = 0; i < 32 * bh; i += 32)
			{
				for (int j = 0; j < 64 * bw; j += 64)
				{
					...
					for (int k = 0; k < 32; k += 8)
					{
						...
						{
							int num2 = 256 * (int)Reform32.tbl32bc[l / 8 + k / 8 * 8];
							for (int m = 0; m < 4; m++)
							{
								int num3 = 64 * m;
								for (int n = 0; n < 16; n++)
								{
									...
									int num7 = (int)(4 * Reform32.tbl32pao[n]) + num3 + num2 + num;
									array[num7] = bin[num6];
		...
		public static void Encode32b(byte[] src, byte[] gsram, int rrw, int rrh, int ddax, int dday, int baseoff, int bw64)
		{
			for (int i = 0; i < rrh; i++)
			{
				...
				for (int j = 0; j < rrw; j++)
				{
					...
					int num4 = 256 * (int)Reform32.tbl32bc[(num2 & 63) / 8 + (num & 31) / 8 * 8];
					int num5 = (num & 7) / 2;
					int num6 = 64 * num5;
					int num7 = (num2 & 7) + 8 * (num & 1);
					int num8 = 4 * (j + rrw * i);
					int num9 = baseoff + (int)(4 * Reform32.tbl32pao[num7]) + num6 + num4 + num3;
					gsram[num9] = src[num8];
		...
		{
			byte[] array = new byte[4 * cx * cy];
			for (int i = 0; i < cy; i++)
			{
				for (int j = 0; j < cx; j++)
				{
					int num = 8192 * (j / 64 + bw64 * (i / 32));
					int num2 = 256 * (int)Reform32.tbl32bc[(j & 63) / 8 + (i & 31) / 8 * 8];
					int num3 = (i & 7) / 2;
					int num4 = 64 * num3;
					int num5 = (j & 7) + 8 * (i & 1);
					int num6 = 4 * (j + cx * i);
					int num7 = readoff + (int)(4 * Reform32.tbl32pao[num5]) + num4 + num2 + num;
